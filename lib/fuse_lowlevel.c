			...
			const struct fuse_compound_result *results)
{
	...
	struct fuse_compound_out header;
	...
	void *buffer = malloc(results_size + header_size);
	if (!buffer) {
		return fuse_reply_err(req, ENOMEM);
	}
	...
	int ret = send_reply_ok(req, buffer, results_size + header_size);

	free(buffer);
	return ret;
}

static void init_compound_ctx(fuse_req_t req, const struct fuse_compound_in *arg, const void *payload)
{
							...
							const void *op_in, const void *in_payload)
{
	...
	init_compound_ctx(req, arg, in_payload);
